    ...
    for row_counter, row in enumerate(active_cells):
        for cell_counter, cell in enumerate(row):
            ...
            if cell:
                ...
                h = row_height - \
                    widget.body.formats[index].t_margin - \
                    widget.body.formats[index].b_margin
                w = cell_width - \
                    widget.body.formats[index].l_margin - \
                    widget.body.formats[index].r_margin
                y = v_margin + row_height * row_counter + \
                    widget.body.formats[index].t_margin
    ...
    def render(self, widget: Widget):
        try:
            ...
            if widget.type_ == 'foreground':
                ...
                sub_windows = create_windows_from_cells(
                    widget, self.window, self.v_margin, self.h_margin)
